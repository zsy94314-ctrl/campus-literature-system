...
本项目可以写“采用 RAG 思想”“RAG 风格流程”“检索增强式综述生成”或“基于本地文献材料的检索增强生成”，但不要写成完整工业级 RAG 平台，也不要写成完全实现标准 RAG 框架。

RAG 在本项目中的体现：

- 用户输入综述主题后，系统先通过智能检索推荐相关文献。
- 用户选择 2 到 5 篇文献作为参考材料。
- 后端基于选中文献的标题、关键词、摘要和正文节选组织生成上下文。
- `rule` 模式基于选中文献内容进行规则归纳，`llm` 模式将选中文献内容作为受约束上下文传给 LLM。
- 后端校验输出完整性，补全或替换参考文献来源，并清理非法引用编号。
- `generationMode` 记录最终生成方式，包括 `rule`、`llm` 和 `llm_fallback_rule`。

RAG 与普通 LLM 自由生成的区别：

- 本系统生成依据来自本地课程项目模拟文献库中的选中文献。
- 参考文献不能由模型自由编造，必须来自用户选择的文献。
- 后端会进行引用编号和参考来源校验，提高生成内容的依据性和可追溯性。

后续展望中可以写工业级 RAG 能力，例如 chunk 切分、向量数据库、段落级引用溯源、Cross-Encoder rerank、流式生成和更严格的事实一致性校验，但这些只能写在展望中。
...
- 补充综述生成借鉴 RAG 思想：先检索相关文献，再基于用户选择的文献生成综述，并由后端校验和补全参考文献来源。
...
- 综述生成不是脱离文献材料的自由生成，而是检索增强式综述生成。
- 先根据主题智能推荐相关文献，再由用户选择 2 到 5 篇文献。
- 后端基于选中文献的标题、关键词、摘要和正文节选生成综述上下文。
- 参考文献来源必须来自用户选择的文献，后端负责校验、补全或替换。
- 不要把该流程写成完整工业级 RAG 系统。
...
- 体现 RAG 思想中的“检索后基于选中文献归纳”，但不调用外部 API。
- 作为稳定兜底方案。
...
- API Key 前端脱敏显示。
- 将选中文献内容作为受约束上下文传给 LLM，避免模型脱离本地文献自由生成。
...
- 离线模式仍体现 RAG 思想：先由智能检索推荐文献，再基于用户选择的本地文献材料生成。
...
- 在线模式是 RAG 风格流程中的增强生成环节，prompt 应基于用户选择的文献材料组织。
...
- LLM 流式输出。
- 工业级 RAG 能力，例如 chunk 切分、向量数据库、段落级引用溯源和跨文献证据对齐。
- Docker 部署。
- API Key 加密存储与密钥轮换。
- 更完善的引用校验与学术格式化。
...
- 不要说系统完全依赖 LLM。
- 不要说 LLM 可以自由生成参考文献。
- 不要把 RAG 思想写成完整工业级 RAG 平台或完全实现标准 RAG 框架。
